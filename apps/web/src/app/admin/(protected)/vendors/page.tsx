...
import { VendorRowActions, type VendorSummary } from "@/sections/admin/VendorRowActions";
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="flex flex-wrap gap-1.5">
          {FILTERS.map((f) => (
            <Link
              key={f.value}
              href={`/admin/vendors?filter=${f.value}`}
              className={
                f.value === active
        ...
        <form className="ml-auto" action="/admin/vendors">
            ...
            {vendors.map((v) => (
              <tr key={v.id} className="transition-colors hover:bg-zinc-50">
                ...
                <td className="px-4 py-3 text-zinc-700">
                  <Link
                    href={`/admin/registrations/${v.registration.id}`}

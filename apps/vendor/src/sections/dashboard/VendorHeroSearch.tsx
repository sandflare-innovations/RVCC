...
export function VendorHeroSearch() {
  ...
  const terms = SEARCH_TERMS.length <= 2 ? [...SEARCH_TERMS, ...SEARCH_TERMS] : SEARCH_TERMS;
      ...
      <div className="pointer-events-none absolute inset-y-0 right-5 left-12 z-10 flex items-center overflow-hidden">
        ...
        <div className="relative h-full min-w-0 flex-1">
          {terms.map((term, idx) => {
            ...
            const isPrevious = idx === (termIndex - 1 + terms.length) % terms.length;

            return (
              <span
                key={`${term}-${idx}`}
                className={`absolute top-1/2 left-0 -mt-[14px] text-lg text-zinc-400 transition-all duration-500 ease-in-out ${
                  value
                    ? "opacity-0"
                    : isActive

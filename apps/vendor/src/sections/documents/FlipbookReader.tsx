...
/** PDFs live on S3/Tigris only (no Vercel /public books). */
function pdfSourceCandidates(doc: DocumentItem): string[] {
  return [doc.fileUrl].filter(Boolean);
}
...
export const FlipbookReader = ({ isOpen, onClose, document: doc }: FlipbookReaderProps) => {
  ...
  // Resolve PDF URL from the public S3/Tigris CDN.
  useEffect(() => {
    if (!doc || !isOpen) return;
    const candidates = pdfSourceCandidates(doc);
    const url = candidates[0] || doc.fileUrl;
    setPdfFile(url);
          ...
          >
            ...
            <div className="scrollbar-thumb-brand-blue/20 flex-1 scrollbar-thin overflow-y-auto p-8 md:p-16">
              <div className="mx-auto max-w-7xl">
                <Document file={pdfFile || doc.fileUrl} options={pdfOptions}>

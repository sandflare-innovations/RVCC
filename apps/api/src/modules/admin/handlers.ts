...
export async function handleRequirementGet(
  ...
  // Return all submitted or processed quotes (not drafts)
  const quotes = await sql`
    SELECT
      q.id, q."newPrice", q.remarks, q."submittedAt", q.status, q."fileName", q."fileUrl",
      v.email AS "participantEmail",
      v.name AS "participantName"
    FROM "Quote" q
    JOIN "VendorUser" v ON v.id = q."vendorUserId"
    WHERE q."requirementId" = ${id} AND q.status != 'DRAFT'
  `;
...
export async function handleQuoteStatusUpdate(
  sql: Sql,
  env: Env,
  request: Request,
  ...
  const body = (await readJson(request)) as { status?: string } | null;
  const status = body?.status;

  if (status !== "SHORTLISTED" && status !== "REJECTED") {
    return json(env, request, { error: "Invalid status. Must be SHORTLISTED or REJECTED." }, 400);
  }

  const [quote] = await sql`
    UPDATE "Quote"
    SET status = ${status}, "updatedAt" = NOW()
    WHERE id = ${quoteId} AND "requirementId" = ${requirementId}
    RETURNING id
  `;

  if (!quote) {
    return json(env, request, { error: "Quote not found for this requirement." }, 404);
  }
  ...
  return json(env, request, { ok: true, status });
}

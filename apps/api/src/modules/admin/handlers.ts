...
function parsePageParam(url: URL): number | null {
  if (!url.searchParams.has("page")) return null;
  const n = Number.parseInt(url.searchParams.get("page") ?? "", 10);
  return Number.isFinite(n) && n >= 1 ? n : 1;
}

function pagePayload<T>(items: T[], total: number, page: number) {
  return {
    items,
    total,
    page,
...
export async function handleRegistrationsList(
  ...
  return json(env, request, page ? pagePayload(items, Number(countRow.count ?? 0), page) : items);
}
...
export async function handleVendorsList(sql: Sql, env: Env, request: Request): Promise<Response> {
  ...
  const url = new URL(request.url);
  const filter = (url.searchParams.get("filter") || "RELEASED").trim();
  const q = (url.searchParams.get("q") || "").trim();
  const like = `%${q}%`;
  const page = parsePageParam(url);
  const take = page ? PAGE_SIZE : 100;
...
export async function handleRequirementsList(
  ...
  const skip = page ? (page - 1) * PAGE_SIZE : 0;
  const [countRow] = page
    ? await sql`SELECT COUNT(*)::int AS count FROM "Requirement"`
    : [{ count: 0 }];
  ...
  return json(env, request, page ? pagePayload(rows, Number(countRow.count ?? 0), page) : rows);
}

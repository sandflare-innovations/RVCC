...
export async function handleQuoteSave(
  ...
  let body: { newPrice?: string | null; remarks?: string; submit?: boolean } = {};
  let fileToUpload: File | null = null;

  if (contentType.includes("multipart/form-data")) {
    const formData = await request.formData().catch(() => new FormData());
    body = {
      newPrice: formData.get("newPrice")?.toString(),
  ...
  if (fileToUpload) {
    try {
      ...
      await fs.writeFile(path.join(uploadDir, uniqueName), Buffer.from(buffer));
      
      fileUrl = `/uploads/quotes/${uniqueName}`;
      fileName = fileToUpload.name;
    } catch (err) {
      console.error("File upload failed", err);
      // Ignore file upload error but log it
    }
  }

  // Retrieve existing quote to keep old file if no new file is uploaded
  const [existingQuote] = await sql`
    SELECT "fileUrl", "fileName" FROM "Quote" WHERE "requirementId" = ${requirementId} AND "vendorUserId" = ${vendor.id}
  `;

  const finalFileUrl = fileUrl || (existingQuote?.fileUrl ?? "");
  const finalFileName = fileName || (existingQuote?.fileName ?? "");

  // The unique constraint makes this an upsert, so a double-clicked submit
  // cannot create two quotes.
  const [saved] = await sql`
    INSERT INTO "Quote"
      (id, "requirementId", "vendorUserId", "newPrice", remarks, status, "submittedAt", "fileName", "fileUrl", "createdAt", "updatedAt")
    VALUES
      (${cuid()}, ${requirementId}, ${vendor.id}, ${price || null}, ${String(body.remarks ?? "")},
       ${submit ? "SUBMITTED" : "DRAFT"}, ${submit ? new Date() : null}, ${finalFileName || null}, ${finalFileUrl || null}, NOW(), NOW())
    ON CONFLICT ("requirementId", "vendorUserId") DO UPDATE SET
      ...
      "updatedAt"   = NOW()
    RETURNING id, status, "newPrice", remarks, "submittedAt", "fileName", "fileUrl"
  `;

...
export async function handleAdminRequest(request: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    const reqQuoteStatus = path.match(/^\/requirements\/([^/]+)\/quotes\/([^/]+)\/status$/);
    if (reqQuoteStatus && request.method === "POST") {
      return await handleQuoteStatusUpdate(
        sql,
        env,
        request,
        decodeURIComponent(reqQuoteStatus[1]!),
        decodeURIComponent(reqQuoteStatus[2]!)
      );
    }

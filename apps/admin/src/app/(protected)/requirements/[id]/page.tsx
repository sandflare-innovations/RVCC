...
import { QuoteActions } from "@/sections/QuoteActions";
                ...
                {ranked.map((q) => (
                  <tr key={q.id} className="transition-colors hover:bg-zinc-50">
                    ...
                    <td className="px-4 py-3">
                      {q.fileName ? (
                        <a href={q.fileUrl ?? "#"} target="_blank" rel="noopener noreferrer" className="text-brand-blue hover:underline font-medium text-xs">
                          {q.fileName}
                        </a>
                      ) : (
                    ...
                    <td className="px-4 py-3">
                      {q.status === "SHORTLISTED" ? (
                        ...
                        <span className="rounded-full bg-zinc-100 px-2.5 py-1 text-xs font-semibold text-zinc-600">
                          {q.status === "SUBMITTED" ? "Submitted" : q.status}
                        </span>
                      )}
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex flex-col gap-2 items-start">
                        ...
                        {!requirement.awardedQuoteId && q.status !== "REJECTED" && q.status !== "SHORTLISTED" && (
                          <QuoteActions 
                            requirementId={requirement.id}
                            quoteId={q.id}
                            vendorLabel={q.vendorEmail}

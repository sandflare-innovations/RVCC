...
export function LogoPulseLoader({
  ...
  return (
    <div
      className={cn("relative flex items-center justify-center", containerSizes[size], className)}
    >
      <div className="bg-brand-blue/20 absolute inset-0 animate-[pulse-dot_2s_ease-in-out_infinite] rounded-lg" />
      <div className="bg-brand-blue/40 absolute inset-2 animate-[pulse-dot_2s_ease-in-out_infinite_0.5s] rounded-lg" />
      <div className="bg-brand-blue relative flex h-full w-full items-center justify-center rounded-lg shadow-lg">
        <span className={cn("font-bold tracking-tighter text-white", textSizes[size])}>R</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div className={cn("flex items-center space-x-1", containerSizes[size], className)}>
      {[...Array(3)].map((_, i) => (
        <div
          key={i}
          className={cn("bg-brand-blue animate-[typing_1s_infinite] rounded-full", dotSizes[size])}
          style={{
...
export function TerminalLoader({
  ...
  return (
    <div className={cn("flex items-center space-x-1", containerSizes[size], className)}>
      <span className={cn("text-brand-blue font-mono", textSizes[size])}>{">"}</span>
      <div
        className={cn("bg-brand-blue animate-[blink_1s_step-end_infinite]", cursorSizes[size])}
      />
      <span className="sr-only">Loading</span>
    </div>
  );
...
export function TextDotsLoader({
  ...
  return (
    <div className={cn("inline-flex items-center", className)}>
      <span className={cn("text-brand-blue font-medium", textSizes[size])}>{text}</span>
      <span className="inline-flex">
        <span className="text-brand-blue animate-[loading-dots_1.4s_infinite_0.2s]">.</span>
        <span className="text-brand-blue animate-[loading-dots_1.4s_infinite_0.4s]">.</span>
        <span className="text-brand-blue animate-[loading-dots_1.4s_infinite_0.6s]">.</span>
      </span>
    </div>
  );
}

export function Loader({ variant = "logo-pulse", size = "md", text, className }: LoaderProps) {
  switch (variant) {
    case "logo-pulse":
      return (
